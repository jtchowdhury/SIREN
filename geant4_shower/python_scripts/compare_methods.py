...
def run_species(interp, sampler, library, pid, n_sample, seed, outdir,
    ...
    for E in energies:
        ...
        # fit fidelity (subsampled -- this is the slow part)
        fl = fit_l2_forms(ens["G4"], x, E, n_fit, rng)
    ...
    _write_fit_csv(fit_rows, os.path.join(outdir, f"compare_fitL2_{name}.csv"))
    _make_all_plots(rows, fit_rows, name, outdir)
...
def _make_all_plots(rows, fit_rows, name, outdir):
    sp = SPECIES_LATEX.get(name, name)
    p = lambda f: os.path.join(outdir, f)

    # 1) absolute RMS  (drop Current Analytic: it is 0 -> invalid on log)
    ...
    # 7) fit fidelity L2 (Andy's): forms vs G4, relative residual
    _one_plot(fit_rows, "fit_l2",
              f"Fit Residual vs Geant4 ({sp})",
              r"$\sum_x (g - \mathrm{fit})^2 \,/\, \sum_x g^2$",
              p(f"fit_residual_l2_{name}.png"),
              method_tags=["analytic", "single", "mixture"], meta=FIT_META,
...
def main():
    ...
    ap.add_argument("--n-fit", type=int, default=150,
                    help="G4 showers subsampled for the fit-fidelity L2 (slow)")
    ...
    lib_species = sorted(PID_TO_NAME.get(p, str(p)) for p in library)
    print(f"species in model  ({model_path}): {model_species}")
    print(f"species in G4 dir ({args.g4_dir}): {lib_species}")
